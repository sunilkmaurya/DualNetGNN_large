...
def gen_normalized_adjs(dataset,transpose=False):
    """
    Return normalized DAD
    """
    N = dataset.graph['num_nodes']
    if transpose == False:
        row, col = dataset.graph['edge_index']
    else:
        col, row = dataset.graph['edge_index']
...
def gen_normalized_adjs_i(dataset,transpose=False):
    """
    Return normalized DAD with self-loop
    """
    N = dataset.graph['num_nodes']
    if transpose == False:
        row, col = dataset.graph['edge_index']
    ...
    row_i = torch.LongTensor(list(range(N)))
    col_i = torch.LongTensor(list(range(N)))
    row = torch.cat([row,row_i])
    col = torch.cat([col,col_i])
    adj = SparseTensor(row=row, col=col, sparse_sizes=(N, N))
    deg = adj.sum(dim=1).to(torch.float)
    D_isqrt = deg.pow(-0.5)
    D_isqrt[D_isqrt == float('inf')] = 0
