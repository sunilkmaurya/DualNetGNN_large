...
def eval_rocauc1(y_pred, y_true):
    """ adapted from ogb
    https://github.com/snap-stanford/ogb/blob/master/ogb/nodeproppred/evaluate.py"""
    print(f"y_ture:{type(y_true)}, y_pred:{type(y_pred)}")
    print(f"y_ture:{y_true.shape}, y_pred:{y_pred.shape}")
    ...
    #return np.mean(roc_auc_score(y_true,y_pred,multi_class='ovr'))


def eval_rocauc(output,labels):
    #preds = output.max(1)[1].type_as(labels)
    output = output[:,1]
    output = output.detach().cpu().numpy()
    output = np.exp(output)
    labels = labels.detach().cpu().numpy()
    return np.mean(roc_auc_score(labels,output))
